...
if (!file.exists(filename)) {
  ...
  # Ensure SAVER output is a plain matrix with correct dimnames
  if (!is.matrix(data_saver)) data_saver <- as.matrix(data_saver)
  if (is.null(rownames(data_saver))) rownames(data_saver) <- rownames(raw_rna)[genes.ind]
  if (is.null(colnames(data_saver))) colnames(data_saver) <- colnames(raw_rna)
