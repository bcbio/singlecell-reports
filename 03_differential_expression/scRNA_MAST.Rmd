...
params:
   ...
   data_subset: out/processed_seurat.qs
   mast_result: out/FULL_MAST_RESULTS_2_age.csv
   column: "age"
...
grpmean <- function(grp, mat) {
  ...
  mat <- mat[names(grp), ]
  grpsum <- rowsum(mat %>% as.matrix(), grp, na.rm = T)
...
```{r load in pre-computed outputs}
topn <- 16
data_subset <- qread(params$data_subset)
MAST_Result <- fread(params$mast_result)
top_DEG <- MAST_Result %>%
...
```{r group mean heatmap,fig.height=6,fig.width=8,results='asis'}
cat("### ", column, "\n\n")
...
```{r volcano_plot,fig.height=10,fig.width=13,results='asis'}
cat("### ", column, "\n\n")
...
```{r sc-heatmap seurat,fig.height=5,fig.width=10}
DotPlot(data_subset,
  features = top_DEG,
...
## (Option 1: Plain) sc-expression across contrast groups: top `r topn` DEGs {.tabset}

```{r sc-expr for all DEG1,fig.height=5,fig.width=10,results='asis'}
...
## (Option 2: Colored) sc-expression across contrast groups: top `r topn` DEGs {.tabset}

```{r sc-expr for all DEG2,fig.height=5,fig.width=10,results='asis'}
relSize <- 1.5
for (g in top_DEG) {
  cat("### ", g, "\n\n")
  p1 <- ggplot(
...
## Options for dot styles for above plots {.tabset}

```{r sc-expr for all DEG3,fig.height=5,fig.width=10,results='asis'}
cat("### ", "quasirandom", "\n\n")
...
```{r sc-expr plot2,fig.height=5,fig.width=10,results='asis'}
cat("### ", "dotplot", "\n\n")
...
```{r sc-expr plot3,fig.height=5,fig.width=10,results='asis'}
cat("### ", "jitter", "\n\n")
p1 <- ggplot(top_n_exp, aes_string(x = "contrast_grp", y = g)) +
  geom_boxplot(width = 0.1, outliers = F) +
  geom_violin(position = position_dodge(1), alpha = 0.5, na.rm = TRUE, trim = FALSE) +
