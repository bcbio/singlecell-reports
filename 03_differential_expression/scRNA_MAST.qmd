...
params:
   ...
   column: "age"
   contrasts: !expr list(c("age", "YOUNG", "OLD"))
   cluster_name: "2"
...
topn <- 16
data_subset <- qread(data_subset)
MAST_Result <- fread(mast_result)
top_DEG <- MAST_Result %>%
...
## (Option 1: Plain) sc-expression across contrast groups: top `r topn` DEGs

::: {.panel-tabset}
```{r sc-expr for all DEG1}
#| fig-height: 5
#| fig-width: 10
#| results: 'asis'
...
## (Option 2: Colored) sc-expression across contrast groups: top `r topn` DEGs

::: {.panel-tabset}
```{r sc-expr for all DEG2}
#| fig-height: 5
#| fig-width: 10
#| results: 'asis'
relSize <- 1.5
