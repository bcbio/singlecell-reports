...
cellchat <- identifyOver!expressedGenes(cellchat) # may take a couple minutes
cellchat <- identifyOver!expressedInteractions(cellchat) # may take a couple minutes
...
```{r create cellchat object rejection}
#| eval: !expr !cellchat_rejection_ran
...
```{r subset and preprocess data rejection}
#| eval: !expr !cellchat_rejection_ran
...
cellchat_grade2 <- identifyOver!expressedGenes(cellchat_grade2) # may take a couple minutes
cellchat_grade2 <- identifyOver!expressedInteractions(cellchat_grade2) # may take a couple minutes

cellchat_grade0 <- subsetData(cellchat_grade0) 
cellchat_grade0 <- updateCellChat(cellchat_grade0)
cellchat_grade0 <- identifyOver!expressedGenes(cellchat_grade0) # may take a couple minutes
cellchat_grade0 <- identifyOver!expressedInteractions(cellchat_grade0) # may take a couple minutes

```

```{r compute communication prob rejection}
#| eval: !expr !cellchat_rejection_ran
